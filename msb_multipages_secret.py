...
#    """, unsafe_allow_html=True)
 ...
 #           st.session_state["secret_unlocked"] = False
    ...
    # Bouton caché derrière le texte "Radar Chart"
    st.markdown("""
        <div style="position: relative;">
            <h2 style="margin-bottom: 20px;">Radar Chart</h2>
    ...
    """.format(button=st.button(" ", key="hidden_button")), unsafe_allow_html=True)

    # Logique d'accès à la page secrète
    if st.session_state.get("hidden_button", False):
        st.session_state["secret_unlocked"] = True
        st.sidebar.title("Menu")
        st.success("Bravo, tu as débloqué la page secrète ! Elle est maintenant dans le menu.")
        st.stop()
    else:
        if "secret_unlocked" not in st.session_state:
            st.session_state["secret_unlocked"] = False
